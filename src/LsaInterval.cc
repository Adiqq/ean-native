...
NAN_METHOD(LsaInterval::GetResult) {
  // unwrap this LsaInterval
  LsaInterval * self = Nan::ObjectWrap::Unwrap<LsaInterval>(info.This());
  ...
  v8::Isolate *isolate = info.GetIsolate();
  v8::Handle<v8::Array> array = v8::Array::New(isolate, self->points.size());
  for(auto i = 0; i < self->points.size(); i++){
    v8::Handle<v8::Array> innerArray = v8::Array::New(isolate, 2);
    boost::numeric::interval<double> result = self->points[i]->x * a1 + a0;
    v8::Local<v8::Number> lower = Nan::New(result.lower());
    v8::Local<v8::Number> upper = Nan::New(result.upper());
    innerArray->Set(0, lower);
    innerArray->Set(1, upper);
    array->Set(i, innerArray);
  }
  
  info.GetReturnValue().Set(array);
}

NAN_METHOD(LsaInterval::GetError) {
  // unwrap this Lsa
  LsaInterval * self = Nan::ObjectWrap::Unwrap<LsaInterval>(info.This());
  ...
  boost::numeric::interval<double> error = 0.0;
  for(auto i = 0; i < self->points.size(); i++){
    IntervalPoint *point = self->points[i];
  ...
  v8::Isolate *isolate = info.GetIsolate();
  v8::Handle<v8::Array> array = v8::Array::New(isolate, argc);
  v8::Local<v8::Number> lower = Nan::New(error.lower());
  v8::Local<v8::Number> upper = Nan::New(error.upper());
  array->Set(0, lower);
  array->Set(1, upper);
